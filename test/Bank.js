...
describe("Bank", function () {
  ...
  it("hack", async function () {
    ...
    const BankAttack = await ethers.deployContract("BankAttack", [Bank]);
    await BankAttack.waitForDeployment();

    const param = {value: ethers.parseEther("0.01")}
    await BankAttack.attackCall1(param);
    await BankAttack.attackCall2(param);
